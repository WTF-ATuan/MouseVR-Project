...
public class ActorWindow : EditorWindow{
	...
	[System.Serializable]
	public class PathMenuSettings : MenuSettings<PathCreator>{ }

	[SerializeField]
	private ActorWindow.ActorMenuSettings actSettings = new ActorWindow.ActorMenuSettings(){ typeName = "Actor" };

	[SerializeField] private ActorWindow.ControllerMenuSettings contSettings = new ActorWindow.ControllerMenuSettings()
			{ typeName = "Controller" };

	[SerializeField]
	private ActorWindow.PathMenuSettings pathSettings = new ActorWindow.PathMenuSettings(){ typeName = "Path" };
	...
	private void OnGUI(){
		...
		Utility.SelectMenu(actSettings);
		...
		// Select and delete.
		EditorGUILayout.BeginHorizontal(LayoutSettings.editWidth);
		Utility.SelectMenu(contSettings);
		...
		if(contSettings.show[1]){
			...
			contSettings.name =
					EditorGUILayout.TextField("Controller Name: ", contSettings.name, LayoutSettings.editFieldOp);
		...
		// Select and delete.
		EditorGUILayout.BeginHorizontal(LayoutSettings.editWidth);
		Utility.SelectMenu(pathSettings);
	...
	private void CreateButton<T>(MenuSettings<T> settings) where T : UnityEngine.Object{
		...
		if(GUILayout.Button("Create", LayoutSettings.buttonOp)){
			...
			if(typeof(T) == typeof(ControllerOutput)){
				//Create Controller. 
				ControllerObject cont =
						(ControllerObject)obj.AddComponent(
							System.Type.GetType(string.Format("Gimbl.{0}", contType.ToString())));
